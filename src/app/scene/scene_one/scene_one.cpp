...
void scene_one::update_scene(const rh::ctx& ctx)
{
...

	{
		...
		auto [sm_view_near, sm_projection_near] = scene_graph_->shadow_map_projection(ctx, sunlight_direction_, shadow_proj, view);

		shadow_proj = glm::perspective(fov, s, g, -500.f / 100);
		auto [sm_view_middle, sm_projection_middle] = scene_graph_->shadow_map_projection(ctx, sunlight_direction_, shadow_proj, view);

		shadow_proj = glm::perspective(fov, s, g, -500.f / 100);
		auto [sm_view_far, sm_projection_far] = scene_graph_->shadow_map_projection(ctx, sunlight_direction_, shadow_proj, view);
