...
VkResult shader_pipeline::build(const VkDevice device)
{
	...
	if (const VkResult result = vkCreateShadersEXT(device, static_cast<uint32_t>(shaders_create_info_.size()), shaders_create_info_.data(), nullptr, shaders.data()); result != VK_SUCCESS) return result;
	{
	...
	{
		const VkPipelineLayoutCreateInfo pl_info = rhi_helpers::create_pipeline_layout_create_info(push_constant_range, 1, layouts.data(), static_cast<uint32_t>(layouts.size()));
